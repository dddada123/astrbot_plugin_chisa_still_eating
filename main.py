...
@register("astrbot_plugin_chisa_still_eating", "Rua432", "3.7.1", "终极跨次元干饭系统")
class FlavorFusionUltimate(Star):
    ...
    def _generate_help_file(self):
        ...
        with open(help_path, "w", encoding="utf-8") as f:
            f.write("【系统扫描报告 - ChisaEating v3.7.1】\n")
            f.write("当前已识别到以下干饭人：\n")
            for name in names:
                f.write(f"- {name}\n")
            f.write("\n如需在 WebUI 指定卡池，请直接复制下方文本到【指定干饭人卡池】配置框：\n")
            f.write(";".join(names) + "\n")
        logging.info(f"[ChisaEating v3.7.1] 📋 已更新可用干饭人清单到 plugin_data 目录，共 {len(names)} 名")
